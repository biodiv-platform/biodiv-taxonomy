...
public class RankController {
  ...
  @Path("all")
  @Operation(
      summary = "Get all the ranks",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of all ranks",
            content =
                @Content(array = @ArraySchema(schema = @Schema(implementation = Rank.class)))),
        @ApiResponse(
            responseCode = "404",
  ...
  @ValidateUser
  @Operation(
      summary = "Add required rank for the taxonomy tree",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Added or updated required rank",
            content = @Content(schema = @Schema(implementation = Rank.class))),
        @ApiResponse(
            responseCode = "404",
            description = "Could not add the rank",
            content = @Content(schema = @Schema(implementation = String.class)))
      })
  public Response addRank(
      @Context HttpServletRequest request,
      @Parameter(description = "Name of the required rank", required = true) @PathParam("rankName")
          String rankName,
      @Parameter(description = "Value of the required rank", required = true)
          @PathParam("rankValue")
          Double rankValue) {
    try {
  ...
  @ValidateUser
  @Operation(
      summary = "Add intermediate rank for the taxonomy tree",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Added or updated intermediate rank",
            content = @Content(schema = @Schema(implementation = Rank.class))),
        @ApiResponse(
            responseCode = "404",
            description = "Could not add the rank",
  ...
  public Response addRank(
      @Context HttpServletRequest request,
      @Parameter(description = "Name of the new rank", required = true) @PathParam("rankName")
          String rankName,
      @Parameter(description = "Higher rank name", required = true) @PathParam("highRankName")
          String highRankName,
      @Parameter(description = "Lower rank name", required = true) @PathParam("lowRankName")
          String lowRankName) {
    try {
