...
public class SpeciesGroupController {
  ...
  @Inject private SpeciesGroupService speciesGroupService;

  @GET
  @Path("taxon")
  @Consumes(MediaType.TEXT_PLAIN)
  @Operation(
      summary = "Get species group from a given taxon id",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Group details",
            content = @Content(schema = @Schema(implementation = SpeciesGroup.class))),
        @ApiResponse(
            responseCode = "404",
  ...
  @Operation(
      ...
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Added species group",
            content = @Content(schema = @Schema(implementation = SpeciesGroup.class))),
        @ApiResponse(
            responseCode = "401",
        ...
        @ApiResponse(
            responseCode = "404",
            description = "Could not add species group",
            content = @Content(schema = @Schema(implementation = String.class)))
      })
    ...
    try {
      if (!TaxonomyUtil.isAdmin(request))
        return Response.status(Status.UNAUTHORIZED)
            .entity("Only admin can add the species group")
            .build();
  ...
  @ValidateUser
  @Operation(
      summary = "Add species group mapping",
      requestBody =
          @RequestBody(
              required = true,
              content = @Content(schema = @Schema(implementation = SpeciesGroupMapping.class))),
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Added species group mapping",
            content = @Content(schema = @Schema(implementation = SpeciesGroupMapping.class))),
        @ApiResponse(
            responseCode = "404",
            description = "Could not add species group mapping",
            content = @Content(schema = @Schema(implementation = String.class)))
      })
  public Response addSpeciesGroupMapping(
      @Context HttpServletRequest request, SpeciesGroupMapping speciesGroupMapping) {
    try {
  ...
  @ValidateUser
  @Operation(
      summary = "Add species permission mapping",
      requestBody =
          @RequestBody(
              required = true,
      ...
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Added species group permission",
            content = @Content(schema = @Schema(implementation = SpeciesPermission.class))),
        @ApiResponse(
            responseCode = "404",
            description = "Could not add species group mapping",
  ...
  public Response addSpeciesPermission(
      @Context HttpServletRequest request, SpeciesPermission speciesPermission) {
    try {
  ...
  @Consumes(MediaType.TEXT_PLAIN)
  @Operation(
      summary = "Find taxonomy by SpeciesGroup ID",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of taxonomy IDs",
            content =
  ...
  public Response getTaxonomyBySpeciesGroup(
      @Parameter(description = "Species Group ID", required = true) @PathParam("speciesGroupId")
          String sGroup,
      @Parameter(description = "Optional taxonomy filter list") @QueryParam("taxonomyList")
  ...
  @Operation(
      summary = "Find all the SpeciesGroup",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "All species groups",
            content =
                @Content(
                    array = @ArraySchema(schema = @Schema(implementation = SpeciesGroup.class)))),
        @ApiResponse(
            responseCode = "404",
            description = "Species Group not found",
            content = @Content(schema = @Schema(implementation = String.class)))
      })
  ...
  @ValidateUser
  @Operation(
      summary = "Get the species permission",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of taxonomy IDs the user can validate",
            content =
                @Content(
                    array =
                        @ArraySchema(schema = @Schema(implementation = SpeciesPermission.class)))),
        @ApiResponse(
            responseCode = "400",
            description = "Unable to get the permission list",
            content = @Content(schema = @Schema(implementation = String.class)))
      })
