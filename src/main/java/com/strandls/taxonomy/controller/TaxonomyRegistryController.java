...
public class TaxonomyRegistryController {
  ...
  @Inject private TaxonomyRegistryService taxonomyRegistry;

  @GET
  @Path(ApiConstants.BREADCRUMB + "/{taxonomyId}")
  @Consumes(MediaType.TEXT_PLAIN)
  @Operation(
      summary = "Find Taxonomy Registry by ID",
      responses = {
        @ApiResponse(
            responseCode = "200",
            ...
            content =
                @Content(
                    array = @ArraySchema(schema = @Schema(implementation = BreadCrumb.class)))),
        @ApiResponse(
            responseCode = "404",
  ...
  @ValidateUser
  @Operation(
      summary = "Find taxon tree for a list of taxons",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of TaxonTree objects",
            content =
                @Content(array = @ArraySchema(schema = @Schema(implementation = TaxonTree.class)))),
        @ApiResponse(
            responseCode = "400",
            description = "Unable to fetch the taxon tree",
            content = @Content(schema = @Schema(implementation = String.class)))
      })
  public Response getTaxonTree(
      @Context HttpServletRequest request,
      @Parameter(description = "Comma-separated list of taxon IDs", example = "123,456,789")
          @QueryParam("taxonList")
          String taxonList) {
    try {
  ...
  @Transactional
  @Operation(
      summary = "Get taxon relationships",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "List of taxon relationships",
            content =
                @Content(
                    array = @ArraySchema(schema = @Schema(implementation = TaxonRelation.class)))),
        @ApiResponse(
            responseCode = "400",
            description = "Unable to fetch the taxon relationship",
            content = @Content(schema = @Schema(implementation = String.class)))
      })
  public Response list(
      @Parameter(description = "Parent taxon ID") @QueryParam("parent") Long parent,
      ...
      @Parameter(description = "Expand taxon", example = "false")
          @DefaultValue("false")
          @QueryParam("expand_taxon")
          Boolean expandTaxon) {
    try {
      List<TaxonRelation> result =
          taxonomyRegistry.list(parent, taxonIds, expandTaxon, classificationId);
      return Response.status(Status.OK).entity(result).build();
    } catch (Exception e) {
  ...
  @ValidateUser
  @Operation(
      summary = "Migrate the taxonomy hierarchy to the clean list",
      description = "Migrate hierarchy following specific order",
      responses = {
        @ApiResponse(
            responseCode = "200",
        ...
        @ApiResponse(
            responseCode = "400",
            description = "Unable to do migration",
            content = @Content(schema = @Schema(implementation = String.class))),
        @ApiResponse(
            responseCode = "401",
      ...
      if (!TaxonomyUtil.isAdmin(request)) {
        throw new WebApplicationException(
            Response.status(Response.Status.UNAUTHORIZED)
                .entity("Only admin can do clean list migration")
                .build());
      }
  ...
  @ValidateUser
  @Operation(
      summary = "Migrate the taxonomy hierarchy for working name",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Migration result",
  ...
  public Response migrateWorkingName(@Context HttpServletRequest request) {
    try {
      if (!TaxonomyUtil.isAdmin(request)) {
        throw new WebApplicationException(
            Response.status(Response.Status.UNAUTHORIZED)
                .entity("Only admin can do working list migration")
                .build());
      }
  ...
  @ValidateUser
  @Operation(
      summary = "Migrate the taxonomy hierarchy for raw name",
      responses = {
        @ApiResponse(
            responseCode = "200",
            description = "Migration result",
  ...
  public Response migrateRawName(@Context HttpServletRequest request) {
    try {
      if (!TaxonomyUtil.isAdmin(request)) {
        throw new WebApplicationException(
            Response.status(Response.Status.UNAUTHORIZED)
                .entity("Only admin can do raw list migration")
                .build());
      }
