...
public class FileMetadata {
  ...
  @JsonIgnore private int scientificColumnIndex = -1;
  @JsonIgnore private int synonymColumnIndex = -1;
  @JsonIgnore private int rankColumnIndex = -1;
  @JsonIgnore private int statusColumnIndex = -1;
  @JsonIgnore private int positionColumnIndex = -1;
  @JsonIgnore private int sourceColumnIndex = -1;
  @JsonIgnore private int sourceIdColumnIndex = -1;
  @JsonIgnore private int commonNameColumnIndex = -1;
  @JsonIgnore private Map<String, Integer> rankToIndex;
  ...
  public FileMetadata(
      String fileType,
      Map<String, String> nameToRank,
      String scientificColumnName,
      String synonymColumnName,
  ...
  public void updateIndices(String[] headers) {
    if (commonNameTagType == null) commonNameTagType = CommonNameTagType.threeLetterCode;
  ...
  public TaxonomySave readOneRow(UtilityServiceApi utilityServiceApi, String[] data)
      throws ApiException {
    TaxonomySave taxonomySave = new TaxonomySave();
    if (scientificColumnIndex == -1
        || rankColumnIndex == -1
        || data[scientificColumnIndex] == null
        || "".equals(data[scientificColumnIndex])
        || data[rankColumnIndex] == null
    ...
    if (statusColumnIndex == -1
        || data[statusColumnIndex] == null
        || "".equals(data[statusColumnIndex])) taxonomySave.setStatus(TaxonomyStatus.ACCEPTED);
    else taxonomySave.setStatus(TaxonomyStatus.fromValue(data[statusColumnIndex].toUpperCase()));

    if (positionColumnIndex == -1
        || data[positionColumnIndex] == null
        || "".equals(data[positionColumnIndex])) taxonomySave.setPosition(TaxonomyPosition.RAW);
    else
      taxonomySave.setPosition(TaxonomyPosition.fromValue(data[positionColumnIndex].toUpperCase()));

    if (synonymColumnIndex != -1) taxonomySave.setSynonyms(data[synonymColumnIndex]);
    if (sourceColumnIndex != -1) taxonomySave.setSource(data[sourceColumnIndex]);
    if (sourceIdColumnIndex != -1) taxonomySave.setSourceId(data[sourceIdColumnIndex]);

    Map<String, String> rankToName = new HashMap<>();
    for (Map.Entry<String, Integer> entry : rankToIndex.entrySet()) {
      String rank = entry.getKey();
      Integer index = entry.getValue();
      if (data[index] == null || "".equals(data[index].trim())) continue;
      rankToName.put(rank, data[index]);
    }
    taxonomySave.setRankToName(rankToName);

    if (commonNameColumnIndex != -1
        && data[commonNameColumnIndex] != null
      ...
      for (String cName : commonNameForLanguage) {
        ...
        if (languageType != null && commonNameString != null) {
          try {
            Language language =
                utilityServiceApi.getLanguage(languageType, commonNameTagType.name());
