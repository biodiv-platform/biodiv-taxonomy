...
@Schema(
    description =
        "Registry entry mapping a taxonomy node to a classification and path using PostgreSQL ltree")
public class TaxonomyRegistry implements Serializable, Cloneable {
  ...
  @SequenceGenerator(
      name = "taxonomy_registry_id_generator",
      sequenceName = "taxonomy_registry_id_seq",
      allocationSize = 1)
  @Column(name = "id")
  public Long getId() {
    return id;
