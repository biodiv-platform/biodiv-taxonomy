...
public class CommonNameDao extends AbstractDAO<CommonName, Long> {
  ...
  public List<CommonName> getCommonName(
    ...
    try (Session session = sessionFactory.openSession()) {
      ...
      if (languageId == null)
        queryStr =
            "from CommonName t where languageId is NULL and taxonConceptId =:taxonConceptId and name =:name and isDeleted = false";
      else
        queryStr =
            "from CommonName t where languageId = :languageId and taxonConceptId =:taxonConceptId and name =:name and isDeleted = false";

      Query<CommonName> query = session.createQuery(queryStr, CommonName.class);
      if (languageId != null) query.setParameter("languageId", languageId);
