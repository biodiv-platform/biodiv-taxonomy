...
public class TaxonomyRegistryDao extends AbstractDAO<TaxonomyRegistry, Long> {
  ...
  private static final Long CLASSIFICATION_ID =
      Long.parseLong(TaxonomyConfig.getString(CLASSIFICATION_ID_STRING));
  ...
  public TaxonomyRegistry findbyTaxonomyId(Long taxonomyId, Long classificationId) {
    ...
    String qry =
        "from TaxonomyRegistry tr where tr.taxonomyDefinationId = :taxonomyId "
  ...
  public TaxonomyRegistry createRegistry(
      Long classificationId,
      String path,
      String rank,
      Long taxonDefinitionId,
      Long uploaderId,
  ...
  public List<String> findByTaxonIdOnTraitList(
      List<Long> traitTaxonIds, Set<String> speciesGroupTaxonIds) {
    return findByTaxonIdOnTraitList(traitTaxonIds, speciesGroupTaxonIds, CLASSIFICATION_ID);
  }

  @SuppressWarnings({"rawtypes", "unchecked"})
  public List<String> findByTaxonIdOnTraitList(
      List<Long> traitTaxonIds, Set<String> speciesGroupTaxonIds, Long classificationId) {

    if (speciesGroupTaxonIds.isEmpty()) return new ArrayList<>();
    ...
    String queryString =
        "select taxon_definition_id from taxonomy_registry t where t.classification_id = :classificationId and "
  ...
  public List<TaxonRelation> list(
      Long parent, List<Long> taxonIds, boolean expandTaxon, Long classificationId) {
    Session session = sessionFactory.openSession();
    String queryString = "";
    Query query;
    if (parent == null && taxonIds == null) {
      queryString =
          "select t.id,t.name, t.rank, ltree2text(tR.path) as path, tR.classification_id as classification, "
  ...
  public List<String> getPathToRoot(List<Long> taxonIds, Long classificationId) {
    ...
    try {
      String sqlString =
          "select cast(taxon_definition_id as varchar) from taxonomy_registry where path @> "
  ...
  public List<TaxonomyRegistryResponse> getPathToRoot(Long taxonId, Long classificationId) {
    ...
    try {
      String sqlString =
          "select cast(td.id as varchar), td.rank, td.name, td.canonical_form from (select * from taxonomy_registry where path @> "
  ...
  public List<TaxonomyRegistry> getOldHierarchy(Long taxonId, List<Long> classificationIds) {
    ...
    try {
      String sqlString =
          "select id, classification_id, ltree2text(path) path, rank, taxon_definition_id, upload_time, uploader_id from taxonomy_registry_backup "
  ...
  @SuppressWarnings("rawtypes")
  public List<TaxonomyRegistryResponse> getPathToRootForOldHierarchy(
      Long taxonId, Long classificationId) {
    Session session = sessionFactory.openSession();
    try {
      String sqlString =
          "select cast(td.id as varchar), td.rank, td.name, td.canonical_form from (select * from taxonomy_registry_backup where path @> "
  ...
  public List<TaxonomyRegistryResponse> getNameFromPath(String path) {
    ...
    try {
      ...
      for (String s : path.split("\\.")) taxonIds.add(Long.parseLong(s));

      String sqlString =
          "select cast(td.id as varchar), td.rank, td.name, td.canonical_form from taxonomy_definition td "
  ...
  /**
   * Order for choosing candidate is as follow 821 - Catalog of Life 820 - FishBase Taxonomy
   * Hierarchy 819 - IUCN Taxonomy Hierarchy (2010) 818 - GBIF Taxonomy Hierarchy 817 - Author
   * Contributed Taxonomy Hierarchy 265798 - Combined Taxonomy Hierarchy
   *
   * <p>If we found any duplicate candidate the we fall back to IBP hierarchy. so we are maintaining
   * both the record Pulling out one of the hierarchy mentioned above in given order and IBP
   * hierarchy
   *
   * <p>265799 - IBP hierarchy
   *
   * <p>Query to execute is as follow
   *
   * <p>select * from taxonomy_registry_backup where taxon_definition_id = :taxonId and
   * (classification_id = :classificationId or classification_id = ( select classification_id from
   * taxonomy_registry_backup where taxon_definition_id = :taxonId group by classification_id order
   * by case classification_id when 821 then 1 when 820 then 2 when 819 then 3 when 818 then 4 when
   * 817 then 5 when 265798 then 6 end limit 1 )) order by classification_id desc
   *
   * @param taxonId
   * @return
   */
  ...
  public TaxonomyRegistry getParentToSnapOn(String path) {
    ...
    try {
      ...
      String sqlString =
          "select * from taxonomy_registry tr "
  ...
  public TaxonomyDefinition findChildWithNotAssigned(Long parentId, Long classificationId) {
    ...
    try {
      ...
      Query<TaxonomyDefinition> query =
          session.createNativeQuery(sqlString, TaxonomyDefinition.class).setMaxResults(1);
      query.setParameter(PARENT_CHECK, parentCheck);
      query.setParameter(CLASSIFICATION_ID_STRING, classificationId);
      List<TaxonomyDefinition> taxonList = query.getResultList();
      if (taxonList.isEmpty()) return null;
      else return taxonList.get(0);
    } catch (Exception e) {
      return null;
    } finally {
      session.close();
