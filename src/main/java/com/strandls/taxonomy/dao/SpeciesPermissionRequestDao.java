...
public class SpeciesPermissionRequestDao extends AbstractDAO<SpeciesPermissionRequest, Long> {
  ...
  public SpeciesPermissionRequest requestPermissionExist(
      Long userId, Long taxonId, TreeRoles roles) {
    String qry =
        "from SpeciesPermissionRequest where userId = :userId and taxonConceptId = :taxonId and role = :role ";
    Session session = sessionFactory.openSession();
    SpeciesPermissionRequest result = null;
    try {
