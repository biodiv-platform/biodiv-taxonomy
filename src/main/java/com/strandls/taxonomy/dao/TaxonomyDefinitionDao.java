...
public class TaxonomyDefinitionDao extends AbstractDAO<TaxonomyDefinition, Long> {
  ...
  public List<Long> getAllIds(int limit, int offset) {
    ...
    List<Long> ids =
        session.createQuery(criteria).setFirstResult(offset).setMaxResults(limit).getResultList();
    session.close();
    return ids;
  }
  ...
  public List<TaxonomyDefinition> breadCrumbSearch(List<Long> taxonIds) {
    ...
    String qry =
        "select t from TaxonomyDefinition t left join Rank r on t.rank = r.name where t.id in(:taxonIds) order by r.rankValue";
    try {
  ...
  public List<Object[]> search(String term) {
    ...
    try {
      String sqlString =
          "select t.name,t.status,t.position,t.id,t.rank from TaxonomyDefinition as t where lower(t.name) like :term order by t.name";
  ...
  public List<String> specificSearch(String term, Long taxonId) {
    ...
    try {
      String sqlString =
          "select distinct(cast(case when t.status = 'SYNONYM' then a.accepted_id else t.id end as varchar)) as id"
      ...
      query.setParameter("term", term.toLowerCase().trim());
      if (taxonId != null) query.setParameter(TAXON_ID, taxonId);
      return query.getResultList();
    } catch (Exception e) {
      logger.error(e.getMessage());
    } finally {
    ...
    try {
      ...
      Query query =
          session
              ...
              .addScalar("taxon_definition_id", StandardBasicTypes.LONG);
      if (taxonId != null) query.setParameter(TAXON_ID, taxonId);
      return query.getResultList();
    } catch (Exception e) {
      logger.error(e.getMessage());
  ...
  public int updateStatusToSynonymInDB(
      TaxonomyRegistry newTaxonomyRegistry, TaxonomyRegistry oldtaxonomyRegistry) {
  ...
  public TaxonomyDefinition createNotAssignedName(
      String rankName, TaxonomyPosition taxonomyPosition, Long uploaderId) {
  ...
  public TaxonomyDefinition createTaxonomyDefiniiton(
  ...
  public TaxonomyNameListResponse getTaxonomyNameList(
    ...
    Query<Integer> countQuery =
        session.createNativeQuery(countQueryString).addScalar("count", StandardBasicTypes.INTEGER);
    ...
    Query<TaxonomyNamelistItem> query =
        session.createNativeQuery(qryString, TaxonomyNamelistItem.class);
