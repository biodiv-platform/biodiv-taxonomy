...
public class TaxonomyRegistryServiceImpl extends AbstractService<TaxonomyRegistry>
    implements TaxonomyRegistryService {
  ...
  @Inject private RankDao rankDao;
  ...
  public List<BreadCrumb> fetchByTaxonomyId(Long id) {
    ...
    if (taxoRegistry == null) {
      ...
      if (acceptedSynonyms != null && !acceptedSynonyms.isEmpty())
        taxoRegistry =
            taxonomyRegistryDao.findbyTaxonomyId(acceptedSynonyms.get(0).getAcceptedId(), null);
    }
    if (taxoRegistry == null) return new ArrayList<>();
  ...
  @Override
  public List<TaxonRelation> list(
      Long parent, String taxonIds, boolean expandTaxon, Long classificationId) {
    List<Long> taxonID;
    if (taxonIds == null || "".equals(taxonIds.trim())) {
      taxonID = null;
    } else {
      taxonID = new ArrayList<>();
      for (String taxon : taxonIds.split(",")) taxonID.add(Long.parseLong(taxon));
    }
    List<String> ids = taxonomyRegistryDao.getPathToRoot(taxonID, classificationId);
    try {
      List<TaxonRelation> inputItems =
          taxonomyRegistryDao.list(parent, taxonID, expandTaxon, classificationId);
  ...
  /**
   * Code below from here on is only for the migration purpose It does the tree migration
   *
  ...
  public Map<String, Object> snapWorkingNames()
      throws CloneNotSupportedException,
          UnRecongnizedRankException,
          ApiException,
          TaxonCreationException {
    String countQueryString =
        "select id from taxonomy_definition where position = :position and status = :status and rank = :rank and is_deleted = false";
    String queryString =
        "from TaxonomyDefinition td where td.position = :position and td.status = :status and rank = :rank and isDeleted = false order by id";
  ...
  private Map<String, Object> snapNames(
      String queryString, Map<String, Object> parameters, String countQueryString) {
    ...
    TaxonomyDefinition rootDefinition =
        taxonomyDefinitionDao.findByCanonicalForm("Root", "root").get(0);

    TaxonomyRegistry root =
        taxonomyRegistryDao.findbyTaxonomyId(rootDefinition.getId(), defaultClassificationId);
    ...
    for (Rank rankToSnap : ranksTopToBottom) {
      ...
      for (int i = 0; i < nameCount; i += BATCH_SIZE) {
        ...
        List<TaxonomyDefinition> definitions =
            taxonomyDefinitionDao.getByQueryString(queryString, parameters, limit, offset);

        for (TaxonomyDefinition definition : definitions) {
          Long userId = definition.getUploaderId();
          List<TaxonomyRegistry> taxonomyRegistrys =
              taxonomyRegistryDao.getSnappingCandidates(definition.getId());
          ...
          if (taxonomyRegistrys.isEmpty()) {
            ...
            isDefaultHierchy =
                defaultClassificationId.equals(taxonomyRegistrys.get(0).getClassificationId());

            candidateToSnap =
                getCandidateToSnap(
                    definition,
                    taxonomyRegistrys,
                    duplicateWithSnapping,
                    duplicateWithoutSnapping,
                    isDefaultHierchy);
            if (candidateToSnap == null) continue;
          }

          // Get Name details for the path
          List<TaxonomyRegistryResponse> nodeWithParents =
              taxonomyRegistryDao.getNameFromPath(candidateToSnap.getPath());
          Set<String> rankNames = new HashSet<>();
          for (TaxonomyRegistryResponse r : nodeWithParents) rankNames.add(r.getRank());
          ...
          TaxonomyRegistry parentToSnapOn =
              taxonomyRegistryDao.getParentToSnapOn(newPath.toString());

          StringBuilder path =
              createRequiredChildren(
                  rootDefinition, parentToSnapOn, nodeWithParents, defaultClassificationId, userId);
          ...
          if (isDefaultHierchy) {
            ...
            if (!defaultHierarchy.equals(path.toString())) {
              ...
              hierarchyMissmatchedStatus.put(
                  "IBP hierarchy name", taxonomyRegistryDao.getNameFromPath(defaultHierarchy));

              hierarchyMissmatchedStatus.put("Snapped hierarchy path", path);
              hierarchyMissmatchedStatus.put(
                  "Snapped hierarchy name", taxonomyRegistryDao.getNameFromPath(path.toString()));

              hierarchyMissmatchedStatus.put("Choosen hierarchy path", newPath.toString());
              hierarchyMissmatchedStatus.put(
                  "Choosen hierarchy name",
                  taxonomyRegistryDao.getNameFromPath(newPath.toString()));
              hierarchyMissmatchedStatus.put(
                  "classification", candidateToSnap.getClassificationId());
  ...
  private StringBuilder createRequiredChildren(
      TaxonomyDefinition definition,
      TaxonomyRegistry parentToSnapOn,
      List<TaxonomyRegistryResponse> nodeWithParents,
      Long defaultClassificationId,
      Long userId) {
    ...
    int index = 0;
    while (!nodeWithParents
        .get(index)
        .getId()
        .equals(parentToSnapOn.getTaxonomyDefinationId().toString())) index++;
  ...
  private TaxonomyRegistry getCandidateToSnap(
      TaxonomyDefinition definition,
      List<TaxonomyRegistry> taxonomyRegistrys,
      Map<String, Object> duplicateWithSnapping,
      Map<String, Object> duplicateWithoutSnapping,
      boolean isDefaultHierchy) {
    ...
    if (isDefaultHierchy) {
      if (size > 2) {
        ...
        duplicateHierarchyStatus.put(
            "Snapping Hierarchy", taxonomyRegistryDao.getNameFromPath(candidateToSnap.getPath()));
  ...
  private StringBuilder createNotAssignedHierarchy(
      TaxonomyDefinition definition,
      List<Rank> ranksBottomToTop,
      List<Rank> ranksTopToBottom,
      Set<String> rankNames,
      List<TaxonomyRegistryResponse> nodeWithParents,
      Long userId) {
    StringBuilder newPath = new StringBuilder();
    ...
    for (Rank rank : ranksTopToBottom) {
      ...
      if (rank.getRankValue() > highestRank) break;
      if (rank.getIsRequired().booleanValue() && !rankNames.contains(rank.getName())) {
        ...
        TaxonomyDefinition notAssignedName =
            taxonomyRegistryDao.findChildWithNotAssigned(parentId, null);
        if (notAssignedName == null) {
          // Create not assigned name.
          notAssignedName =
              taxonomyDefinitionDao.createNotAssignedName(rank.getName(), position, userId);
        }
      ...
      if (!"".equals(newPath.toString())) newPath.append(".");
  ...
  public Map<String, Object> migrateCleanName() throws CloneNotSupportedException {
    ...
    String countQueryString =
        "select id from taxonomy_definition where position = :position and status = :status and is_deleted = false";
    String queryString =
        "from TaxonomyDefinition td where td.position = :position and td.status = :status and isDeleted = false order by id";
    ...
    for (int i = 0; i < cleanNameCount; i += BATCH_SIZE) {
      ...
      for (TaxonomyDefinition definition : definitions) {
        ...
        List<TaxonomyRegistry> taxonomyRegistrys =
            taxonomyRegistryDao.getOldHierarchy(definition.getId(), classificationIds);

        if (taxonomyRegistrys.isEmpty()) {
          if (definition.getIsDeleted().booleanValue()) continue;
          Map<String, Object> r = new HashMap<>();
          ...
          List<TaxonomyRegistryResponse> generatedHierarchyName =
              taxonomyRegistryDao.getPathToRootForOldHierarchy(
          ...
          List<TaxonomyRegistryResponse> actualHierarchyName =
              taxonomyRegistryDao.getNameFromPath(path);

          // Check for the valid hierarchy - See if all the required ranks are present.
          Set<String> rankNames = new HashSet<>();
          for (TaxonomyRegistryResponse r : actualHierarchyName) rankNames.add(r.getRank());
