...
public class SpeciesGroupServiceImpl extends AbstractService<SpeciesGroup>
    implements SpeciesGroupService {

  private final Logger logger = LoggerFactory.getLogger(SpeciesGroupServiceImpl.class);

  @Inject private SpeciesGroupMappingDao speciesMappingDao;

  @Inject private SpeciesGroupDao speciesGroupDao;
  ...
  public List<String> fetchBySpeciesGroupId(Long id, List<String> traitTaxonList) {
    ...
    if (traitTaxonList.isEmpty()) return new ArrayList<>();
    List<Long> traitTaxonIds = new ArrayList<>();
    for (String taxonId : traitTaxonList) traitTaxonIds.add(Long.parseLong(taxonId));
    List<String> result =
        taxonomyRegistryDao.findByTaxonIdOnTraitList(traitTaxonIds, speciesGroupTaxons);
