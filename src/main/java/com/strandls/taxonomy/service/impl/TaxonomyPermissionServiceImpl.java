...
public class TaxonomyPermissionServiceImpl implements TaxonomyPermisisonService {
  ...
  @Inject private EncryptionUtils encryptUtils;

  @Inject private TaxonomyRegistryService registryService;

  @Inject private SpeciesPermissionDao speciesPermissionDao;

  @Inject private SpeciesPermissionRequestDao permissionReqDao;

  @Inject private MailUtils mailUtils;

  @Inject private TaxonomyDefinitionDao taxDefinationDao;

  @Inject private UserServiceApi userService;

  @Inject private EsUserSpeciesPermissionUpdate userPermissionUpdate;
  ...
  public Boolean getPermissionOnTree(HttpServletRequest request, Long taxonId) {
    ...
    for (BreadCrumb crumb : breadcrumbs) {
      ...
      permission =
          speciesPermissionDao.checkPermission(userId, crumb.getId(), TreeRoles.SPECIESCONTRIBUTOR);
      if (permission.booleanValue()) break;

      //			for taxonomy contrbutor role
      permission =
          speciesPermissionDao.checkPermission(
              userId, crumb.getId(), TreeRoles.TAXONOMYCONTRIBUTOR);
      if (permission.booleanValue()) break;
    }
  ...
  public Boolean assignUpdatePermissionDirectly(
    ...
    try {
      ...
      if (userRole.contains("ROLE_ADMIN")) {
        ...
        SpeciesPermission hasPermission =
            speciesPermissionDao.findPermissionOntaxon(
                permissionData.getUserId(), permissionData.getTaxonId());

        //				deleting the req if already it was raised
        SpeciesPermissionRequest isExist =
            permissionReqDao.requestPermissionExist(
                permissionData.getUserId(), permissionData.getTaxonId(), role);
        if (isExist != null) {
        ...
        if (hasPermission == null) {
          ...
          SpeciesPermission speciesPermission =
              new SpeciesPermission(
                  null,
                  0L,
                  permissionData.getUserId(),
                  new Date(),
                  roleIdMap.get(role),
        ...
        mailUtils.sendPermissionGrant(
            requestee, taxDef.getName(), role.getValue(), permissionData.getTaxonId());
        return true;
      }
      return false;
    } catch (Exception e) {
  ...
  public Boolean requestPermission(HttpServletRequest request, PermissionData permissionData) {
    ...
    Boolean alreadyHasPermission =
        speciesPermissionDao.checkPermission(userId, permissionData.getTaxonId(), role);
    if (alreadyHasPermission.booleanValue()) return false;

    SpeciesPermissionRequest isExist =
        permissionReqDao.requestPermissionExist(userId, permissionData.getTaxonId(), role);
    if (isExist == null) {
      SpeciesPermissionRequest permissionRequest =
          new SpeciesPermissionRequest(null, permissionData.getTaxonId(), userId, role.getValue());
  ...
  private void sendMail(SpeciesPermissionRequest permissionReq, String requestorMessage) {
    ...
    try {
      ...
      mailUtils.sendPermissionRequest(
          requestors,
          taxDef.getName(),
          taxDef.getId(),
          role.getValue(),
  ...
  public Boolean verifyPermissionGrant(HttpServletRequest request, EncryptedKey encryptedKey) {
    ...
    try {
      ...
      if (userRoles.contains("ROLE_ADMIN")) {
        ...
        SpeciesPermissionRequest permissionReq =
            om.readValue(reqdata, SpeciesPermissionRequest.class);
        SpeciesPermissionRequest permissionReqOriginal =
            permissionReqDao.findById(permissionReq.getId());
        TreeRoles role = TreeRoles.valueOf(permissionReqOriginal.getRole().replace(" ", ""));
        if (permissionReqOriginal.equals(permissionReq)) {

          SpeciesPermission alreadyExist =
              speciesPermissionDao.findPermissionOntaxon(
                  permissionReqOriginal.getUserId(), permissionReqOriginal.getTaxonConceptId());
          if (alreadyExist == null) {
            SpeciesPermission permission =
                new SpeciesPermission(
                    null,
                    0L,
                    permissionReqOriginal.getUserId(),
                    new Date(),
                    roleIdMap.get(role),
          ...
          mailUtils.sendPermissionGrant(
              requestee, taxDef.getName(), role.getValue(), permissionReq.getTaxonConceptId());
  ...
  public Boolean checkIsObservationCurator(HttpServletRequest request, Long taxonomyId) {
    ...
    for (BreadCrumb crumb : breadcrumbs) {
      ...
      permission =
          speciesPermissionDao.checkPermission(userId, crumb.getId(), TreeRoles.OBSERVATIONCURATOR);
      if (permission.booleanValue()) return permission;
    }
    return permission;
  }
}
