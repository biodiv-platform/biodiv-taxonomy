...
public abstract class AbstractDAO<T, K extends Serializable> {
  ...
  public Long getRowCount(String queryString, Map<String, Object> parameters) {
    try (Session session = sessionFactory.openSession()) {
      ...
      NativeQuery<Long> countQuery =
          session.createNativeQuery(countQueryStr).addScalar("count", StandardBasicTypes.LONG);
      parameters.forEach(countQuery::setParameter);
      return countQuery.getSingleResult();
    }
  }

  public List<T> getByQueryString(
      String hql, Map<String, Object> parameters, int limit, int offset) {
    try (Session session = sessionFactory.openSession()) {
