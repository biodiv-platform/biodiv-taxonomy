...
public class EsUserSpeciesPermissionUpdate {
  ...
  @Inject private RoleServiceApi roleService;

  @Inject private SpeciesPermissionDao speciesPermissionDao;

  @Inject private EsServicesApi esService;
  ...
  public void speciesUserPermissionEsUpdate(Long userId) throws ApiException {
    ...
    roles.forEach(
        role -> {
          List<SpeciesPermission> list =
              speciesPermissionDao.getTaxonIdByUserAndRole(userId, role.getId());
          List<Long> taxonList =
              list.stream().map(x -> x.getTaxonConceptId()).collect(Collectors.toList());
          UserTaxonRoleMapping taxonRoleMapping =
              new UserTaxonRoleMapping(userId, role.getId(), role.getAuthority(), taxonList);
          if (!taxonList.isEmpty()) ugRoleMapping.add(taxonRoleMapping);
        });
